...
from easi.tasks.ebhabitat.vendor.config.default_structured_configs import (
...
class EBHabEnv(gym.Env):
    def __init__(self, eval_set='train', exp_name='', down_sample_ratio=1.0,
        ...
        Args:
            data_dir: Path to simulator assets (scenes, objects, robots).
                      If provided, a 'data' symlink is created for Habitat config resolution.
            dataset_dir: Path to directory containing {split}.pickle files.
                         Defaults to vendor/datasets/ if not provided.
        """
        # load config
        ...
        if dataset_dir:
            self.config.habitat.dataset.data_path = os.path.join(dataset_dir, '{}.pickle'.format(eval_set))
        else:
            self.config.habitat.dataset.data_path = os.path.join(os.path.dirname(__file__), 'datasets/{}.pickle'.format(eval_set))
        # Create 'data' symlink for Habitat scene asset resolution
        if data_dir:
            from pathlib import Path
            data_link = Path('data')
            if not data_link.exists():
                try:
                    os.symlink(os.path.abspath(data_dir), str(data_link))
                except OSError:
                    logger.warning("Could not create 'data' symlink to %s", data_dir)
