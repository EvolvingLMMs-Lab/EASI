"""EB-Alfred bridge subprocess for AI2-THOR v2.1.0.
...
Extends the generic AI2ThorBridge with EB-Alfred-specific behavior:
- High-level skill execution (find, pick up, put down, open, close, etc.)
- State tracking (cleaned, cooled, heated objects)
- Goal condition evaluation

Ported from EmbodiedBench:
- envs/eb_alfred/thor_connector.py (high-level skill API)
- envs/eb_alfred/env/thor_env.py (scene restoration, state tracking)
...
from easi.tasks.ebalfred.thor_utils import (
    evaluate_goal_conditions,
    load_task_json_with_repeat,
)

logger = logging.getLogger("easi.bridge.ebalfred")


class EBAlfredBridge(AI2ThorBridge):
    """EB-Alfred bridge extending AI2ThorBridge with skill execution and goal evaluation.

    Ported from EmbodiedBench's ThorEnv + ThorConnector.
    """

    def __init__(self, workspace, data_dir=None, simulator_kwargs=None):
        super().__init__(workspace, data_dir, simulator_kwargs)
        self.traj_data = None
    ...
    def reset(self, reset_config):
        ...
        if not task_path:
            # Smoke test mode: use generic reset
            self.traj_data = None
