"""EB-Alfred goal evaluation and task loading utilities.

Moved from easi/simulators/ai2thor/v2_1_0/thor_utils.py — these are
task-specific (EB-Alfred), not generic AI2-THOR helpers.

Ported from EmbodiedBench:
- envs/eb_alfred/env/tasks.py (goal evaluation)
- envs/eb_alfred/utils.py (task loading)
...
# numpy is available in the ai2thor conda env (Python 3.8)
# but may not be in the host venv — import is optional for importability
try:
    import numpy as np
except ImportError:
...
logger = logging.getLogger("easi.bridge.ebalfred_utils")
