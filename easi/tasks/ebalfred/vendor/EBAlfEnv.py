...
import easi.tasks.ebalfred.vendor.utils as utils
from easi.tasks.ebalfred.vendor.utils import alfred_objs, alfred_open_obj, alfred_pick_obj, alfred_slice_obj, alfred_open_obj, alfred_toggle_obj, alfred_recep
from easi.tasks.ebalfred.vendor.thor_connector import ThorConnector
...
class EBAlfEnv(gym.Env):
    ...
    def __init__(self, exp_name='', detection_box=False, resolution=500, data_dir=None, x_display=None, max_steps=30):
        ...
        Args:
            ...
            detection_box: whether to draw detection boxes on images
        ...
        # data_dir = path to EASI's downloaded HF dataset task JSONs
        # Falls back to bundled path if None
        self._data_dir = data_dir or ALFRED_DATASET_PATH
        self.resolution = resolution
        display = x_display if x_display is not None else X_DISPLAY
        self.env = ThorConnector(x_display=display, player_screen_height=resolution, player_screen_width=resolution)
    ...
    def reset(self, episode=None):
        ...
        Args:
            episode: dict with 'task', 'repeat_idx', 'instruction' keys
                     (from EASI's HF dataset via bridge reset_config)

        Returns:
            observation
        """
        assert episode is not None, "episode dict required for reset"
        self._reset_controller(episode)
        self._current_step = 0
        self._cur_invalid_actions = 0
        obs = {
...
if __name__ == "__main__":
    ...
    Requires an episode dict with 'task', 'repeat_idx', 'instruction' keys.
    """
    env = EBAlfEnv()
    episode = {
